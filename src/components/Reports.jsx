...
export default function Reports({ members, payments, monthKeys, expenses, totalCollected, allTimeCollected, balance }) {
  ...
  const exportCSV = () => {
    const rows = [['Member', 'Phone', 'Status', 'Amount (BDT)', 'Payment Date']]
    members.forEach(m => {
      const p = payments[sel]?.[m.id]
      rows.push([m.name, m.phone, p ? 'Paid' : 'Unpaid', p ? DONATION_AMOUNT : 0, p?.date || '-'])
    })
    const csv = rows.map(r => r.join(',')).join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' }))
    a.download = `Team71_Fund_${sel}.csv`
    a.click(); a.remove()
  }
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.card}>
        <div style={S.cardTitle}>📋 {fmtMonth(sel)} Summary</div>
        ...
        <Row label='Amount Collected' val={`৳${totalCollected(sel)}`} />
        <Row label='Amount Due'       val={`৳${unpaidM.length * DONATION_AMOUNT}`} vc='#dc2626' />
        <Row label='Collection Rate'  val={`${members.length ? Math.round((paidM.length / members.length) * 100) : 0}%`} />
        <button onClick={exportCSV} style={{ ...S.primaryBtn, marginTop: 10, background: '#2563eb' }}>
          ⬇ Export CSV
        </button>
      </div>
      ...
      {unpaidM.length > 0 && (
        <div style={{ ...S.card, border: '1.5px solid #fecaca' }}>
          ...
          {unpaidM.map(m => (
            <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #fff5f5' }}>
              <span style={{ fontSize: 13 }}>{m.name}</span>
              <span style={{ fontSize: 13, color: '#dc2626', fontWeight: 700 }}>৳{DONATION_AMOUNT}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {paidM.length > 0 && (
        <div style={{ ...S.card, border: '1.5px solid #bbf7d0' }}>
          ...
          {paidM.map(m => (
            <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f0fdf4' }}>
              <span style={{ fontSize: 13 }}>{m.name}</span>
              <span style={{ fontSize: 13, color: '#16a34a', fontWeight: 700 }}>
                ৳{DONATION_AMOUNT} · {payments[sel]?.[m.id]?.date}
              </span>
            </div>
          ))}
        </div>
